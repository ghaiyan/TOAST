...
def train_gae(adj_matrix, hidden_dim, output_dim, num_epochs=200, lr=0.001, device='cuda'):
    input_dim=adj_matrix.shape[0]
    model = GAE(input_dim, hidden_dim, output_dim).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        encoded, adj_hat = model(adj_matrix)
    ...
    # 使用最小重构误差的模型和嵌入表示Z作为最终结果
    model.load_state_dict(best_model)
    final_encoded_input_adj_matrix = best_z.to("cpu").detach().numpy()

    return final_encoded_input_adj_matrix,train_losses

import csv
def save_to_csv(data, file_path):
    with open(file_path, 'w', newline='') as csvfile:
        csv_writer = csv.writer(csvfile)
        csv_writer.writerow(["Epoch", "Reconstruction Loss"])
...
res=40000
chr="sim"
datadir="/data1/ghy_data/toast/simulation/"
hidden_dim=64
output_dim = 16
todir="/data1/ghy_data/toast/simulation/200epoch/dim="+str(output_dim)+"/"

#Hi-C file dir
for noise in [4,8,12,16,20]:
    hic=datadir+str(noise)+'noise.hic'
    #sub_matrix dir
    matDir=todir+str(noise)+'noise'
    #result dir
    dir=matDir
    ...
    #quality file 
    qualityFile=dir+'/Toast_quality-metrics-'+str(chr)+'.txt'
    ...
    for i in range(len(sub_matrices)):
        ...
        for lr in [0.001]:
            if torch.cuda.is_available():
                # 使用CUDA进行训练
                embedding,train_losses = train_gae(adj_matrix, hidden_dim, output_dim, num_epochs=200, lr=0.001, device='cuda:4')
            else:
                # 使用CPU进行训练
                embedding,train_losses = train_gae(adj_matrix, hidden_dim, output_dim, num_epochs=200, lr=0.001, device='cpu')
            #embedding,train_losses = train_gae(adj_matrix, hidden_dim, output_dim, num_epochs=1000, lr=0.001)
            save_to_csv(train_losses, dir+"/train_losses.csv")
